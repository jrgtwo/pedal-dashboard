    ...
    <main className="overflow-scroll w-full">
      ...
      <section className="flex flex-wrap mt-4">
        {pedalList && pedalList.map((item) => {
          ...
          return (
            <div
              className="w-[200px] h-[200px] relative group p-4 hover:bg-gray-200 rounded-2xl gap-8  overflow-hidden"
              key={`${item.name}:${item.id}`}>
              {isOwned && (
                <div className="content-['Owned'] absolute bg-amber-500 rotate-[-45deg] text-white text-xs px-12 pt-12 pb-2 top-[-28px] left-[-60px] ">Owned</div>
              )}
